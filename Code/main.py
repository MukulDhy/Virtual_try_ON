...
# parser.add_argument('--openpose_model', default='pose_iter_584000.caffemodel', help='Path to OpenPose .caffemodel model was trained on COCO dataset.')
...
class BilinearFilter(object):
    ...
    def _precompute_coeffs(self, inSize, outSize):
        ...
        # for i, row in enumerate(bounds[1::2]):
        #     adjusted_row = np.arange(row) + xmins[i]
        #     adjusted_row /= filterscale
        #     points.append(adjusted_row)
        ...
        for i, row in enumerate(bounds[1::2]):
            adjusted_row = np.arange(row) + xmins[i]
            adjusted_row /= filterscale
            points_list.append(adjusted_row)

        # Find the maximum length of rows in points_list
        max_length = max(len(row) for row in points_list)

        # Pad shorter rows with zeros to make them of the same length
        points = np.array([np.pad(row, (0, max_length - len(row))) for row in points_list])

        # Now you can divide the entire array by filterscale
        points /= filterscale
